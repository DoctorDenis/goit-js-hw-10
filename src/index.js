...
function renderCountriesList(countries) {
  ...
  document.querySelectorAll('.countries-item').forEach(element => element.addEventListener("click", event => {
    const substr = event.currentTarget.querySelector("p").textContent.split(" ");
  ...
  .then((countries) => {
    if (countries.length > 1 && countries.length < 10) {
      renderCountriesList(countries);
          ...
          refs.listEl.innerHTML = "";
  refs.countryDivEl.innerHTML = "";
      Notify.info(`There is too many countries found. Please specify the search query!`);
    } else {
      renderCountry(countries[0]);
    }    
  })
...
function renderCountry(country) {
  ...
  refs.countryDivEl.innerHTML = `
    ...
    <p class="country-languages"><b>Languages:</b> ${country.languages.map(language => `${language.name}`).join(", ")}</p>`
}